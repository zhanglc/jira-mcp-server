...
export class JiraClientWrapper {
  ...
  constructor(config: JiraConfig) {
    ...
    if (config.bearer) {
      ...
      logger.log('Jira client initialized with Bearer Token', {
        host: new URL(config.url).hostname,
      });
    } else if (config.username && config.password) {
      clientConfig.username = config.username;
      clientConfig.password = config.password;
      logger.log('Jira client initialized with Basic Auth', {
        host: new URL(config.url).hostname,
  ...
  async getIssueTransitions(issueKey: string, fields?: string[]): Promise<JiraTransition[]> {
    try {
      ...
      if (!response.transitions) {
        logger.log(
          `No transitions property in response for issue: ${issueKey}`
        );
        return [];
      }
      ...
      logger.log(
        `Successfully retrieved ${transitions.length} transitions for issue: ${issueKey}`
      );
  ...
  ): Promise<SearchResult<JiraIssue>> {
    try {
  ...
  async getAllProjects(includeArchived?: boolean, fields?: string[]): Promise<JiraProject[]> {
    try {
      ...
      if (includeArchived === false) {
        ...
        logger.log(
          `Filtered out archived projects, ${projects.length} active projects remaining`
        );
      }
  ...
  async getCurrentUser(fields?: string[]): Promise<JiraUser> {
    try {
      ...
      logger.log(
        `Successfully retrieved current user: ${user.displayName} (${user.name})`
      );
  ...
  async getIssueWorklogs(issueKey: string, fields?: string[]): Promise<JiraWorklog[]> {
    try {
      ...
      logger.log(
        `Successfully retrieved ${worklogs.length} worklogs for issue: ${issueKey}`
      );
  ...
  async getProjectIssues(
    ...
    try {
      ...
      logger.log(
        `Successfully retrieved ${result.total} issues for project: ${projectKey}`
      );
  ...
  async getProjectVersions(projectKey: string, fields?: string[]): Promise<JiraVersion[]> {
    try {
      ...
      if (!Array.isArray(response)) {
        logger.log(
          `Response is not an array for project versions: ${projectKey}`
        );
        return [];
      }
      ...
      logger.log(
        `Successfully retrieved ${versions.length} versions for project: ${projectKey}`
      );
  ...
  async getProject(projectKey: string, fields?: string[]): Promise<JiraProject> {
    try {
      ...
      if (
        ...
        logger.error(
          `Invalid project response - missing required fields for: ${projectKey}`,
          project
        );
        throw new ApiError(
          `Invalid project information received for: ${projectKey}`,
          500
        );
      }

      logger.log(
        `Successfully retrieved project: ${project.name} (${project.key})`
      );
  ...
  async getUserProfile(username: string, fields?: string[]): Promise<JiraUser> {
    try {
      ...
      if (!user.self || !user.name || !user.displayName || !user.emailAddress) {
        logger.error(
          `Invalid user response - missing required fields for: ${username}`,
          user
        );
        throw new ApiError(
          `Invalid user information received for: ${username}`,
          500
        );
      }

      logger.log(
        `Successfully retrieved user profile: ${user.displayName} (${user.name})`
      );
  ...
  async getAgileBoards(projectKey?: string, fields?: string[]): Promise<JiraBoard[]> {
    try {
      ...
      if (projectKey !== undefined && projectKey.trim() !== '') {
        ...
        logger.log(
          `Filtered to ${boards.length} boards for project: ${trimmedProjectKey}`
        );
      }
  ...
  async getBoardIssues(
    ...
    try {
      ...
      logger.log(
        `Successfully found ${result.total} issues for board: ${boardId}`
      );
  ...
  async getSprintsFromBoard(boardId: number, fields?: string[]): Promise<JiraSprint[]> {
    try {
      ...
      if (response.values === undefined) {
        logger.log(
          `No values property in board sprints response for board: ${boardId}`
        );
        return [];
      }
      ...
      if (!Array.isArray(response.values)) {
        logger.log(
          `Values property is not an array in board sprints response for board: ${boardId}`
        );
        return [];
      }
      ...
      logger.log(
        `Successfully retrieved ${sprints.length} sprints for board: ${boardId}`
      );
  ...
  async getSprint(sprintId: number, fields?: string[]): Promise<JiraSprint> {
    try {
      ...
      if (!sprint.id || !sprint.self || !sprint.state || !sprint.name) {
        logger.error(
          `Invalid sprint response - missing required fields for: ${sprintId}`,
          sprint
        );
        throw new ApiError(
          `Invalid sprint information received for: ${sprintId}`,
          500
        );
      }

      logger.log(
        `Successfully retrieved sprint: ${sprint.name} (${sprint.state})`
      );
  ...
  async getSprintIssues(
    ...
    try {
      ...
      logger.log(
        `Successfully found ${result.total} issues for sprint: ${sprintId}`
      );
  ...
  async downloadAttachments(issueKey: string, fields?: string[]): Promise<JiraAttachment[]> {
    try {
      ...
      if (!issue) {
        ...
        throw new ApiError(
          `No issue information received for: ${issueKey}`,
          500
        );
      }
      ...
      if (!issue.fields) {
        logger.error(
          `Invalid issue response - missing fields for: ${issueKey}`,
          issue
        );
        throw new ApiError(
          `Invalid issue response - missing fields for: ${issueKey}`,
      ...
      logger.log(
        `Successfully retrieved ${attachments.length} attachments for issue: ${issueKey}`
      );
  ...
  async getSystemInfo(fields?: string[]): Promise<JiraSystemInfo> {
    try {
      ...
      if (
        !systemInfo.baseUrl ||
        !systemInfo.version ||
        !systemInfo.versionNumbers ||
        !systemInfo.deploymentType ||
        systemInfo.buildNumber === undefined ||
        !systemInfo.buildDate ||
        !systemInfo.scmInfo
      ) {
        logger.error(
          'Invalid system information response - missing required fields',
          systemInfo
        );
        throw new ApiError(
      ...
      logger.log(
        `Successfully retrieved system info: ${systemInfo.deploymentType} ${systemInfo.version}`
      );
  ...
  async getServerInfo(fields?: string[]): Promise<JiraServerInfo> {
    try {
      ...
      if (
        !serverInfo.baseUrl ||
        !serverInfo.version ||
        !serverInfo.versionNumbers ||
        !serverInfo.deploymentType ||
        serverInfo.buildNumber === undefined ||
        !serverInfo.buildDate ||
        !serverInfo.serverTime ||
        !serverInfo.scmInfo
      ) {
        logger.error(
          'Invalid server information response - missing required fields',
          serverInfo
        );
        throw new ApiError(
      ...
      logger.log(
        `Successfully retrieved server info: ${serverInfo.deploymentType} ${serverInfo.version}`
      );
