...
export class ToolHandler {
  ...
  private async handleGetIssue(args: ToolArgs): Promise<ToolResponse> {
    try {
      ...
      const issue = await this.jiraClient!.getIssue(issueKey, fields);
   ...
   */
  private async handleGetIssueTransitions(args: ToolArgs): Promise<ToolResponse> {
    try {
      const { issueKey } = args;
  ...
  private async handleSearchIssues(args: ToolArgs): Promise<ToolResponse> {
    try {
      ...
      if (startAt !== undefined && (typeof startAt !== 'number' || startAt < 0)) {
   ...
   */
  private async handleDownloadAttachments(args: ToolArgs): Promise<ToolResponse> {
    try {
      const { issueKey } = args || {};
  ...
  private async handleGetProjectIssues(args: ToolArgs): Promise<ToolResponse> {
    try {
      ...
      const result = await this.jiraClient!.getProjectIssues(projectKey, options);
   ...
   */
  private async handleGetProjectVersions(args: ToolArgs): Promise<ToolResponse> {
    try {
      const { projectKey } = args;
  ...
  private async handleGetCurrentUser(args: ToolArgs): Promise<ToolResponse> {
    try {
      if (args && Object.keys(args).length > 0) {
        throw new ApiError('getCurrentUser does not accept any parameters', 400);
      }
   ...
   */
  private async handleGetSprintsFromBoard(args: ToolArgs): Promise<ToolResponse> {
    try {
      const { boardId } = args;
