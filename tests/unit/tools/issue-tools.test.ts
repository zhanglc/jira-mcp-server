...
describe('Issue Tools Module', () => {
      ...
      tools.forEach(tool => {
        ...
        if (tool.inputSchema.required) {
          tool.inputSchema.required.forEach((requiredField: string) => {
            expect(tool.inputSchema.properties as any).toHaveProperty(
              requiredField
            );
          });
        }
  ...
  describe('Schema Validation', () => {
    ...
    it('should have consistent pagination parameters in searchIssues', () => {
      ...
      expect(
        (tool.inputSchema.properties as any).startAt.description
      ...
      expect(
        (tool.inputSchema.properties as any).maxResults.description
      ).toContain('Maximum number');
      expect(
        (tool.inputSchema.properties as any).maxResults.description
      ).toContain('default: 50');
    });
    ...
    it('should have consistent fields parameter structure', () => {
      ...
      toolsWithFields.forEach(tool => {
        ...
        expect(fieldsProp.description).toContain('Field selection');
      });
    });
  });
});
