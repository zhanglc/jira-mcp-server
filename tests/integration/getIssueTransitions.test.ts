...
describe('getIssueTransitions Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    if (!process.env.JIRA_URL || !process.env.JIRA_PERSONAL_TOKEN) {
      console.warn('Skipping integration tests: JIRA_URL or JIRA_PERSONAL_TOKEN not configured');
      return;
    }
  ...
  describe('real Jira Server API integration', () => {
    ...
    beforeEach(() => {
      ...
      if (!process.env.JIRA_URL || !process.env.JIRA_PERSONAL_TOKEN) {
        pending('Integration tests require JIRA_URL and JIRA_PERSONAL_TOKEN environment variables');
      }
    });
    ...
    it('should retrieve transitions for valid issue and validate response structure', async () => {
      ...
      console.log(`Retrieved ${transitions.length} transitions for issue: ${TEST_ISSUE_KEY}`);
      ...
      if (transitions.length > 0) {
        ...
        transitions.forEach((transition: JiraTransition, index: number) => {
          ...
          console.log(`Transition ${index + 1}: ${transition.id} - ${transition.name} -> ${transition.to.name}`);
        });
      } else {
        console.log(`No transitions available for issue: ${TEST_ISSUE_KEY} (issue may be in final state)`);
      }
    ...
    it('should handle non-existent issue with proper error', async () => {
      ...
      await expect(jiraClient.getIssueTransitions(nonExistentIssue))
        .rejects
        .toThrow(ApiError);
      ...
      try {
        ...
        console.log(`Correctly handled non-existent issue error: ${(error as ApiError).message}`);
      }
    }, 10000);
  ...
  describe('JiraTransition interface compliance', () => {
    ...
    it('should validate TypeScript interface compliance with real data', async () => {
      ...
      const transitions: JiraTransition[] = await jiraClient.getIssueTransitions(TEST_ISSUE_KEY);
      ...
      console.log(`✅ All ${transitions.length} transitions comply with JiraTransition interface`);
    }, 10000);
  });
});