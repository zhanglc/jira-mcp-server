...
describe('MCP Server - getAgileBoards Tool Integration', () => {
  ...
  beforeAll(() => {
    ...
    if (!token) {
      console.log('Skipping MCP Server getAgileBoards integration tests - JIRA_PERSONAL_TOKEN not set');
      return;
    }
  ...
  describe('getAgileBoards MCP Tool', () => {
    ...
    it('should filter boards by project key via MCP handler', async () => {
      ...
      const response = await (mcpServer as any).handleGetAgileBoards({ projectKey: 'DSCWA' });
      ...
      console.log(`✓ Project filtering working - found ${boards.length} boards for DSCWA`);
    }, 30000);

    it('should return empty array for non-existent project via MCP handler', async () => {
      // Act - Test with non-existent project
      const response = await (mcpServer as any).handleGetAgileBoards({ projectKey: 'NONEXISTENT_XYZ123' });
      ...
      console.log('✓ Non-existent project correctly returns empty array via MCP');
    }, 30000);

    it('should handle invalid projectKey type via MCP handler', async () => {
      // Act & Assert
      await expect((mcpServer as any).handleGetAgileBoards({ projectKey: 123 }))
    ...
    it('should handle empty string projectKey via MCP handler', async () => {
      ...
      const response = await (mcpServer as any).handleGetAgileBoards({ projectKey: '' });
      ...
      console.log(`✓ Empty project key handled correctly - retrieved ${boards.length} boards`);
    }, 30000);
  });
});