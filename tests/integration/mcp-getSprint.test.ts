...
describe('MCP Server - getSprint Tool Integration', () => {
  ...
  beforeAll(() => {
    ...
    if (!token) {
      console.log('Skipping MCP Server getSprint integration tests - JIRA_PERSONAL_TOKEN not set');
      return;
    }
  ...
  describe('getSprint MCP Tool', () => {
    it('should retrieve sprint details via MCP handler', async () => {
      ...
      const sprintsResponse = await (mcpServer as any).handleGetSprintsFromBoard({ boardId: testBoardId });
      ...
      if (sprints.length === 0) {
        console.log('ℹ️ Board has no sprints (possibly Kanban board) - skipping getSprint test');
        return;
      }
      ...
      const response = await (mcpServer as any).handleGetSprint({ sprintId: testSprintId });
    ...
    it('should validate MCP getSprint data consistency with getSprintsFromBoard', async () => {
      ...
      for (const board of boards.slice(0, 3)) { // Test up to 3 boards
        try {
          const sprintsResponse = await (mcpServer as any).handleGetSprintsFromBoard({ boardId: board.id });
          const sprints = JSON.parse(sprintsResponse.content[0].text);
          
          if (sprints.length > 0) {
            testSprintId = sprints[0].id;
            boardSprintData = sprints[0];
            console.log(`📋 Testing consistency with board: ${board.name} (Sprint ${testSprintId})`);
            break;
          }
        } catch (error) {
      ...
      const sprintResponse = await (mcpServer as any).handleGetSprint({ sprintId: testSprintId });
      ...
      if (boardSprintData.completeDate && individualSprintData.completeDate) {
        expect(individualSprintData.completeDate).toBe(boardSprintData.completeDate);
      }
      
      if (boardSprintData.originBoardId && individualSprintData.originBoardId) {
        expect(individualSprintData.originBoardId).toBe(boardSprintData.originBoardId);
      }
      ...
      console.log(`✓ MCP getSprint data consistent with getSprintsFromBoard for sprint ${testSprintId}`);
    }, 45000);
    ...
    it('should handle errors properly via MCP', async () => {
      ...
      console.log(`✓ MCP getSprint correctly handled non-existent sprint ID: ${nonExistentSprintId}`);
    }, 30000);

    it('should validate MCP input parameters', async () => {
      // Test missing sprintId
      await expect(
        (mcpServer as any).handleGetSprint({})
      ).rejects.toThrow();
    ...
    it('should handle multiple sprint requests efficiently', async () => {
      ...
      if (boards.length === 0) {
        console.log('⚠️ No agile boards found - skipping multiple requests test');
        return;
      }
      ...
      console.log(`🏃 Testing multiple getSprint requests with ${testSprintIds.length} sprints`);
      ...
      responses.forEach((response, index) => {
        ...
        console.log(`✓ Sprint ${testSprintIds[index]}: ${sprint.name} (${sprint.state})`);
      });

      console.log(`✓ Successfully handled ${testSprintIds.length} concurrent getSprint requests`);
    }, 60000);
  });
});