...
describe('JiraClientWrapper.getSprint - Integration Tests', () => {
  ...
  describe('real API integration', () => {
    it('should retrieve individual sprint details successfully', async () => {
      ...
      if (sprints.length === 0) {
        console.log('ℹ️ Board has no sprints (possibly Kanban board) - skipping individual sprint test');
        return;
      }

      // Test with the first sprint
      const testSprintId = sprints[0].id;
      console.log(`🏃 Testing getSprint with sprint ID: ${testSprintId} (${sprints[0].name})`);
      ...
      if (sprint.completeDate) {
        ...
        expect(new Date(sprint.completeDate).toString()).not.toBe('Invalid Date');
    ...
    it('should validate data consistency with getSprintsFromBoard', async () => {
      ...
      if (testSprints.length === 0) {
        console.log('ℹ️ No sprints found in any boards - skipping consistency test');
        return;
      }

      console.log(`📋 Testing data consistency with board: ${testBoardName} (${testSprints.length} sprints)`);
      ...
      for (const boardSprint of testSprints) {
        try {
          ...
          if (boardSprint.completeDate && individualSprint.completeDate) {
            expect(individualSprint.completeDate).toBe(boardSprint.completeDate);
          }
          
          if (boardSprint.originBoardId && individualSprint.originBoardId) {
            expect(individualSprint.originBoardId).toBe(boardSprint.originBoardId);
          }
          ...
          console.log(`✓ Sprint ${boardSprint.id} (${boardSprint.name}): Data consistent between methods`);
        } catch (error) {
          console.log(`⚠️ Could not get individual sprint ${boardSprint.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    ...
    it('should handle different sprint states correctly', async () => {
      ...
      for (const board of boards.slice(0, 5)) { // Test up to 5 boards
        try {
          const sprints = await client.getSprintsFromBoard(board.id);
          
          for (const sprint of sprints.slice(0, 2)) { // Test up to 2 sprints per board
            try {
              ...
              if (stateResults.active.length > 0 && stateResults.closed.length > 0 && stateResults.future.length > 0) {
                break;
              }
            } catch (error) {
      ...
      Object.entries(stateResults).forEach(([state, sprints]) => {
        ...
        if (sprints.length > 0) {
          ...
          if (state === 'closed' && example.completeDate) {
            ...
            console.log(`    ✓ Closed sprint has complete date: ${example.completeDate}`);
          }
          
          if (state === 'active' && example.startDate) {
            expect(typeof example.startDate).toBe('string');
            console.log(`    ✓ Active sprint has start date: ${example.startDate}`);
          }
        }
      });
      ...
      console.log(`✓ Successfully tested getSprint with ${totalSprints} sprints across different states`);
    ...
    it('should validate sprint data model against JiraSprint interface', async () => {
      ...
      console.log(`📋 Validating sprint data model from board: ${testBoardName} (${testSprints.length} sprints)`);
      ...
      for (const sprintId of testSprints) {
        try {
          ...
          console.log(`✓ Sprint ${sprintId} (${sprint.name}): Data model valid`);
        } catch (error) {
          console.log(`⚠️ Could not validate sprint ${sprintId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
          // Don't fail the test if individual sprints can't be accessed
        }
      }
  ...
  describe('error handling', () => {
    it('should throw ApiError for non-existent sprint', async () => {
      ...
      await expect(client.getSprint(nonExistentSprintId)).rejects.toThrow(ApiError);
    ...
    it('should handle permission denied gracefully', async () => {
      ...
      for (const sprintId of testSprintIds) {
        try {
          ...
          console.log(`✓ Sprint ${sprintId}: Error handled properly - ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  describe('edge cases with real data', () => {
    it('should handle large sprint IDs correctly', async () => {
      ...
      console.log(`📊 Testing with largest found sprint ID: ${largestSprintId}`);

      try {
        const sprint = await client.getSprint(largestSprintId);
        expect(sprint.id).toBe(largestSprintId);
        expect(typeof sprint.id).toBe('number');
        console.log(`✓ Large sprint ID ${largestSprintId} handled correctly: ${sprint.name}`);
      } catch (error) {
        // If we can't access this specific sprint, that's still a valid test
        expect(error).toBeInstanceOf(ApiError);
        console.log(`✓ Large sprint ID ${largestSprintId} error handled properly: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }, 45000);
    ...
    it('should handle sprints from different boards', async () => {
      ...
      const testedSprints: { boardId: number; boardName: string; sprintId: number; sprintName: string; status: string }[] = [];
      ...
      testedSprints.forEach(result => {
        console.log(`📋 Board ${result.boardName} (${result.boardId}): Sprint ${result.sprintId} - ${result.status}`);
      });

      // Assert that we tested at least some sprints
      expect(testedSprints.length).toBeGreaterThan(0);
      console.log(`✓ Tested getSprint with ${testedSprints.length} sprints from different boards`);
