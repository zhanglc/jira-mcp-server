...
describe('MCP Server - getIssueWorklogs Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    if (!token) {
      console.log('Skipping MCP Server getIssueWorklogs integration tests - JIRA_PERSONAL_TOKEN not set');
      return;
    }
  ...
  describe('getIssueWorklogs MCP Tool', () => {
    it('should return issue worklogs when called via MCP handler', async () => {
      ...
      const response = await (mcpServer as any).handleGetIssueWorklogs({ issueKey: TEST_ISSUE_KEY });
      ...
      if (worklogs.length > 0) {
        ...
        console.log(`✅ MCP getIssueWorklogs tool returned ${worklogs.length} worklog(s) for issue ${TEST_ISSUE_KEY}`);
      } else {
    ...
    it('should reject requests with missing issueKey parameter', async () => {
      ...
      await expect((mcpServer as any).handleGetIssueWorklogs({}))
    ...
    it('should reject requests with invalid issueKey type', async () => {
      ...
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: 123 }))
        .rejects.toThrow('issueKey is required and must be a string');
    });

    it('should handle non-existent issue gracefully', async () => {
      // Act & Assert - Test the handler directly with non-existent issue
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: 'NONEXISTENT-99999' }))
        .rejects.toThrow();
    }, 10000);
    ...
    it('should provide complete worklog data for time tracking analysis', async () => {
      ...
      if (worklogs.length > 0) {
        ...
        console.log(`Total time logged: ${totalTimeSeconds} seconds (${Math.round(totalTimeSeconds / 3600 * 100) / 100} hours)`);
        ...
        console.log(`ℹ No time tracking data available for issue ${TEST_ISSUE_KEY}`);
      }
    }, 15000);
    ...
    it('should validate parameter validation works correctly', async () => {
      ...
      const validResponse = await (mcpServer as any).handleGetIssueWorklogs({ issueKey: TEST_ISSUE_KEY });
      ...
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: null }))
        .rejects.toThrow('issueKey is required and must be a string');

      // Act & Assert - Test undefined issueKey (should fail)
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: undefined }))
        .rejects.toThrow('issueKey is required and must be a string');

      // Act & Assert - Test empty string issueKey (should fail)
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: '' }))
        .rejects.toThrow('issueKey is required and must be a string');
      ...
      console.log('✅ Parameter validation working correctly for getIssueWorklogs');
    }, 10000);
  });
  ...
  describe('MCP Tool Definition', () => {
    it('should include getIssueWorklogs in MCP server definition', async () => {
      ...
      console.log('✅ getIssueWorklogs handler successfully defined in MCP Server');
    });
    ...
    it('should have correct tool schema definition', async () => {
      ...
      console.log('✅ getIssueWorklogs tool schema correctly integrated into MCP Server');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully through MCP interface', async () => {
      ...
      await expect((mcpServer as any).handleGetIssueWorklogs({ issueKey: 'INVALID_FORMAT' }))
        .rejects.toThrow();
