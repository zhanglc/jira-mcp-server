...
describe('JiraClientWrapper.downloadAttachments Integration Tests', () => {
  ...
  describe('Real Jira Server Integration', () => {
    ...
    it('should throw ApiError for non-existent issue', async () => {
      ...
      await expect(
        wrapper.downloadAttachments('NONEXISTENT-999999')
    ...
    it('should handle malformed issue key appropriately', async () => {
      ...
      await expect(
        wrapper.downloadAttachments('INVALID_FORMAT')
      ).rejects.toThrow(ApiError);
    }, 30000);
    ...
    it('should validate attachment URL format', async () => {
      ...
      if (result.length > 0) {
        result.forEach(attachment => {
          ...
          expect(attachment.content).toMatch(
            /^https?:\/\/.*\/secure\/attachment\/\d+\/.+/
          );

          // If thumbnail exists, it should also be properly formatted
          if (attachment.thumbnail) {
            expect(attachment.thumbnail).toMatch(
              /^https?:\/\/.*\/secure\/(thumbnail|attachment)\/\d+\/.+/
            );
          }
        });
    ...
    it('should handle issue with specific attachment types', async () => {
      ...
      if (result.length > 0) {
        ...
        console.log(
          `Found ${images.length} images, ${documents.length} documents, ${others.length} other files`
        );

        // Images should potentially have thumbnails
        images.forEach(img => {
          console.log(
            `Image: ${img.filename} (${img.mimeType}), has thumbnail: ${!!img.thumbnail}`
          );
        });
    ...
    it('should handle issues with large numbers of attachments', async () => {
      ...
      if (result.length > 10) {
        console.log(
          `Issue has ${result.length} attachments - testing large attachment list handling`
        );
        ...
        result.forEach((attachment, index) => {
          ...
          if (index < 3) {
            ...
            console.log(
              `Attachment ${index + 1}: ${attachment.filename} (${attachment.size} bytes)`
            );
          }
        });
      } else {
        console.log(
          `Issue has ${result.length} attachments (not testing large list scenario)`
        );
      }
    }, 30000);
