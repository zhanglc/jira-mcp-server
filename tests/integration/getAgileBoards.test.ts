...
describe('JiraClientWrapper.getAgileBoards - Integration Tests', () => {
  ...
  describe('Basic functionality with real Jira Server', () => {
    ...
    it('should handle boards with different optional properties', async () => {
      ...
      const boardsWithEditPermission = boards.filter(
        board => board.canEdit !== undefined
      );
      ...
      console.log(
        `✓ Boards with edit permission info: ${boardsWithEditPermission.length}`
      );
      ...
      if (boardsWithLocation.length > 0) {
        ...
        if (boardWithLocation.location?.type === 'project') {
          ...
          console.log(
            `✓ Sample project board: ${boardWithLocation.name} -> ${boardWithLocation.location.projectKey}`
          );
        }
      }
    }, 30000);
    ...
    it('should validate board type distribution', async () => {
      ...
      Object.keys(boardTypes).forEach(type => {
        expect(['scrum', 'kanban', 'simple', 'next-gen']).toContain(
          type.toLowerCase()
        );
      });
    }, 30000);
  ...
  describe('Project filtering functionality', () => {
    it('should filter boards by DSCWA project if available', async () => {
      ...
      if (dscwaBoards.length > 0) {
        console.log(
          `✓ Sample DSCWA board: ${dscwaBoards[0].name} (${dscwaBoards[0].type})`
        );
      }
    }, 30000);
    ...
    it('should handle case-sensitive project matching', async () => {
      ...
      const allBoards = await client.getAgileBoards();
      const projectBoard = allBoards.find(
        board =>
          board.location?.type === 'project' && board.location?.projectKey
      );
      ...
      if (projectBoard && projectBoard.location?.projectKey) {
        ...
        const lowercaseMatch = await client.getAgileBoards(
        ...
        console.log(
          `✓ Case sensitivity test: ${projectKey} found ${exactMatch.length} boards`
        );
        console.log(
          `✓ Case sensitivity test: ${projectKey.toLowerCase()} found ${lowercaseMatch.length} boards`
        );
      } else {
  ...
  describe('Data model validation', () => {
    it('should validate complete board model against real data', async () => {
      ...
      const completeBoard = boards.find(
        board =>
          board.location &&
          (board.admins ||
            board.canEdit !== undefined ||
            board.isPrivate !== undefined)
      );
      ...
      if (completeBoard) {
        ...
        console.log(
          `✓ Project: ${completeBoard.location?.projectKey || 'N/A'}`
        );
      } else {
        console.log(
          '⚠ No boards with complete information found for detailed validation'
        );
      }
    }, 30000);
    ...
    it('should validate board self URLs are accessible', async () => {
      ...
      sampleBoards.forEach(board => {
        expect(board.self).toMatch(
          /^https?:\/\/.+\/rest\/agile\/1\.0\/board\/\d+$/
        );
  ...
  describe('Performance and pagination handling', () => {
    it('should handle large number of boards efficiently', async () => {
      ...
      console.log(
        `✓ Performance: ${((boards.length / duration) * 1000).toFixed(2)} boards/second`
      );
    }, 30000);
  });
  ...
  describe('Error scenarios with real environment', () => {
    it('should handle agile functionality gracefully if not available', async () => {
      ...
      try {
        ...
        console.log(
          `✓ Agile functionality not available: ${apiError.message} (${apiError.statusCode})`
        );
      }
    }, 30000);
  });
