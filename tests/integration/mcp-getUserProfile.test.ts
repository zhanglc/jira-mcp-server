...
describe('MCP getUserProfile Tool Integration Tests', () => {
  ...
  describe('getUserProfile MCP Tool - Real Jira Server Tests', () => {
    it('should retrieve user profile through MCP server with current user email', async () => {
      ...
      const result = await mcpServer['handleGetUserProfile']({ username: currentUserEmail });
    ...
    it('should return consistent data between getUserProfile and getCurrentUser MCP tools', async () => {
      ...
      const userProfileResult = await mcpServer['handleGetUserProfile']({ username: currentUser.emailAddress });
      ...
      console.log('✅ Consistent data between getCurrentUser and getUserProfile MCP tools');
    });
    ...
    it('should validate Jira Server field format in MCP response', async () => {
      ...
      // Act
      const userProfileResult = await mcpServer['handleGetUserProfile']({ username: currentUserEmail });
      ...
      console.log('✅ MCP Server field format validation passed for user:', user.key);
    });
    ...
    it('should handle MCP tool parameter validation', async () => {
      ...
      try {
        ...
        throw new Error('Expected getUserProfile to throw an error for missing username');
      } catch (error: any) {
      ...
      try {
        ...
        throw new Error('Expected getUserProfile to throw an error for invalid username type');
      } catch (error: any) {
        expect(error.message).toContain('username is required and must be a string');
        expect(error.statusCode).toBe(400);
      }
    ...
    it('should demonstrate MCP tool functionality for user management', async () => {
      ...
      if (userProfile.applicationRoles) {
        console.log(`   Application Roles: ${userProfile.applicationRoles.size} total`);
      }
