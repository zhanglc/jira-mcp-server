...
describe('JiraClientWrapper.getSprintIssues - Integration Tests', () => {
    ...
    it('should retrieve sprint issues from a real sprint successfully', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping sprint issues tests'
          );
          return;
        }
        throw error;
      ...
      if (sprints.length === 0) {
        console.log(
          '⚠️ No sprints found in board - skipping sprint issues tests'
        );
        return;
      }
      ...
      console.log(
        `🏃 Testing with sprint: ${testSprintName} (ID: ${testSprintId}, State: ${sprints.find(s => s.id === testSprintId)?.state})`
      );
      ...
      console.log(
        `✓ Retrieved ${result.total} issues from sprint ${testSprintId}`
      );
      ...
      if (result.total > 0 && result.issues.length > 0) {
        ...
        console.log(
          `✓ Project: ${firstIssue.fields.project.key} - ${firstIssue.fields.project.name}`
        );
      } else {
    ...
    it('should retrieve sprint issues with pagination', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping pagination test'
          );
          return;
        }
        throw error;
      ...
      for (const sprint of sprints) {
        try {
          ...
          if (quickCheck.total > 5) {
            // Sprint has enough issues for pagination test
      ...
      if (!testSprintId) {
        console.log(
          'ℹ️ No sprint with enough issues found for pagination test'
        );
        return;
      }
      ...
      if (result.total > 3) {
        ...
        console.log(
          `✓ Pagination working: showing ${result.issues.length} of ${result.total} issues`
        );
      } else {
        expect(result.issues.length).toBe(result.total);
        console.log(
          `✓ Sprint has ${result.total} issues (less than maxResults)`
        );
      }
    }, 30000);
    ...
    it('should retrieve sprint issues with field selection', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping field selection test'
          );
          return;
        }
        throw error;
      ...
      if (result.issues.length > 0) {
        ...
        console.log(
          `✓ Field selection working: ${Object.keys(firstIssue.fields).length} fields returned`
        );
    ...
    it('should validate sprint issues data model against SearchResult<JiraIssue> interface', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping data model validation'
          );
          return;
        }
        throw error;
      ...
      if (!testResult) {
        console.log(
          'ℹ️ No sprint with issues found - skipping data model validation'
        );
        return;
      }
      ...
      console.log(
        `✓ All ${testResult.issues.length} issues validated against SearchResult<JiraIssue> interface`
      );
    }, 45000);
    ...
    it('should compare getSprintIssues results with searchIssues for consistency', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping consistency test'
          );
          return;
        }
        throw error;
      ...
      const searchIssuesResult = await client.searchIssues(
        `sprint = ${testSprintId}`
      );

      // Assert results are consistent
      expect(sprintIssuesResult.total).toBe(searchIssuesResult.total);
      expect(sprintIssuesResult.issues.length).toBe(
        searchIssuesResult.issues.length
      );

      // Compare issue keys to ensure same issues are returned
      const sprintIssueKeys = sprintIssuesResult.issues
        .map(issue => issue.key)
        .sort();
      const searchIssueKeys = searchIssuesResult.issues
        .map(issue => issue.key)
        .sort();
      ...
      console.log(
        `✓ Consistency verified: both methods return ${sprintIssuesResult.total} issues`
      );
  ...
  describe('error handling', () => {
    it('should throw ApiError for non-existent sprint', async () => {
      ...
      await expect(client.getSprintIssues(nonExistentSprintId)).rejects.toThrow(
        ApiError
      );
    ...
    it('should handle sprint with no issues gracefully', async () => {
      ...
      try {
        ...
        if (
          ...
          console.log(
            'ℹ️ Board does not support sprints (likely Kanban) - skipping empty sprint test'
          );
          return;
        }
        throw error;
      ...
      try {
        ...
        expect(result.issues.length).toBe(
          Math.min(result.total, result.maxResults)
        );

        console.log(
          `✓ Sprint ${testSprintId}: ${result.total} issues (handled gracefully)`
        );
      } catch (error) {
        // If an error is thrown, it should be an ApiError
        expect(error).toBeInstanceOf(ApiError);
        console.log(
          `✓ Sprint ${testSprintId}: Error handled properly - ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }, 30000);
  });
    ...
    it('should handle different sprint states (active, closed, future)', async () => {
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              console.log(
                `⚠️ Board ${board.name} does not support sprints (likely Kanban) - skipping`
              );
              continue;
            }
            throw error;
          ...
          if (sprints.length > 0) {
            ...
            for (const state of ['active', 'closed', 'future']) {
              ...
              if (sprintsInState.length > 0) {
                ...
                try {
                  const result = await client.getSprintIssues(testSprint.id, {
                    maxResults: 5,
                  });
                  stateTests.push({
                    state: state,
          ...
          console.log(
            `⚠️ Could not test board ${board.name}: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
      ...
      testResults.forEach(result => {
        ...
        result.stateTests.forEach(test => {
          if (test.success) {
            console.log(
              `  ${test.state}: ${test.sprintName} - ${test.issueCount} issues`
            );
          } else {
            console.log(
              `  ${test.state}: ${test.sprintName} - Error: ${test.error}`
            );
          }
        });
      });
      ...
      console.log(
        `✓ Tested ${totalStateTests.length} sprint states across ${testResults.length} boards`
      );
