...
describe('MCP getSprintIssues Tool Integration Tests', () => {
  ...
  describe('successful getSprintIssues tool execution', () => {
    it('should retrieve sprint issues through MCP tool interface', async () => {
      ...
      if (boards.length === 0) {
        console.log('⚠️ No agile boards found - skipping MCP getSprintIssues test');
        return;
      }
      ...
      for (const board of boards.slice(0, 5)) { // Check up to 5 boards
        try {
      ...
      if (!testSprintId) {
        console.log('⚠️ No sprints found in any boards - skipping MCP getSprintIssues test');
        return;
      }

      console.log(`🏃 Testing MCP tool with sprint: ${testSprintName} (ID: ${testSprintId})`);
      ...
      const getSprintIssuesTool = listToolsResponse.tools.find((tool: any) => tool.name === 'getSprintIssues');
      expect(getSprintIssuesTool).toBeDefined();
      expect(getSprintIssuesTool.name).toBe('getSprintIssues');
      expect(getSprintIssuesTool.description).toContain('Get issues from a specific sprint');
      ...
      const resultData = JSON.parse(callToolResponse.content[0].text);
      ...
      console.log(`✓ MCP tool returned ${resultData.total} issues from sprint ${testSprintId}`);
    ...
    it('should handle getSprintIssues with pagination options through MCP tool', async () => {
      ...
      if (!testSprintId) {
        console.log('⚠️ No sprint with issues found - skipping MCP pagination test');
        return;
      }
      ...
      if (resultData.total > 3) {
        ...
        console.log(`✓ MCP pagination working: showing ${resultData.issues.length} of ${resultData.total} issues`);
      } else {
        expect(resultData.issues.length).toBe(resultData.total);
        console.log(`✓ Sprint has ${resultData.total} issues (less than maxResults)`);
      }
    }, 30000);
    ...
    it('should handle getSprintIssues with field selection through MCP tool', async () => {
      ...
      if (boards.length === 0) {
        console.log('⚠️ No agile boards found - skipping MCP field selection test');
        return;
      }
      ...
      if (!testSprintId) {
        console.log('⚠️ No sprint with issues found - skipping MCP field selection test');
        return;
      }

      console.log(`🏃 Testing MCP field selection with sprint: ${testSprintId}`);
      ...
      if (resultData.issues.length > 0) {
        ...
        console.log(`✓ MCP field selection working: ${Object.keys(firstIssue.fields).length} fields returned`);
  ...
  describe('MCP tool schema validation', () => {
    it('should have correct tool schema in ListTools response', async () => {
      ...
      expect(getSprintIssuesTool).toBeDefined();
      expect(getSprintIssuesTool.name).toBe('getSprintIssues');
      expect(getSprintIssuesTool.description).toBe('Get issues from a specific sprint with optional pagination and field selection');
      ...
      expect(schema.properties.sprintId.description).toBe('The numeric ID of the sprint');
      
      expect(schema.properties.startAt).toBeDefined();
      expect(schema.properties.startAt.type).toBe('number');
      expect(schema.properties.startAt.description).toBe('Starting index for pagination (default: 0)');
      
      expect(schema.properties.maxResults).toBeDefined();
      expect(schema.properties.maxResults.type).toBe('number');
      expect(schema.properties.maxResults.description).toBe('Maximum number of results to return (default: 50)');
      ...
      expect(schema.properties.fields.description).toBe('Optional array of field names to retrieve');
